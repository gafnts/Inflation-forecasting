...
La última medida a considerar será el error medio absoluto escalado (MASE), que fue propuesta por primera vez en @hyndman2006 con el objeto de brindar una mejor opción que el error porcentual absoluto medio simétrico (sMAPE). Esta medida está definida de la siguiente manera:

$$MASE=\frac{1}{k}\frac{\sum_{t=1}^{k}|y_{t}-\hat{y}_{t}|}{\frac{1}{n-m}\sum_{t=1}^{k}|y_{t}-\hat{y}_{t-m}|}
$$

donde $n$ es el número de observaciones históricas disponibles y $m$ es la frecuencia de la serie de tiempo. MASE, entre otras características, también es fácil de interpretar: Un $MASE < 1$ indica que los pronósticos del modelo evaluado son, en promedio, más exactos que los pronósticos del modelo de referencia [@makridakis2018].

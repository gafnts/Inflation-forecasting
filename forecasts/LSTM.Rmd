...
```{r}
lstm <- function(assess, epochs, ...) {
  
  # Training the model
  lstm_training <- function(assess, epochs, ...) {
    
    # Cross validation
    splits <- data %>% time_series_split(index, assess = assess, cumulative = TRUE)
    df_trn <- training(splits)
    df_tst <- testing(splits)
    
    df <- 
      bind_rows(df_trn %>% add_column(key = "training"),
                df_tst %>% add_column(key = "testing")) %>% 
      as_tbl_time(index = index)
    ...
    pred_tbl <- tibble(index = lag_test_tbl$index,
                       value = (pred_out * scale_history + center_history))
  ...
  safe_lstm <- possibly(lstm_training, otherwise = NA)
  safe_lstm(split, epochs, ...)
}
```

```{r}
lstm("24 months", epochs = 300)
```

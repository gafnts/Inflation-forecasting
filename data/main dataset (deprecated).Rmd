...
First, I removed inflation expectations because the series started at 2010.\
Second, what's better: itcer or exchange?
...
imae %<>% 
  select(fecha,
         "imae" = imae_variacion_interanual_2)
...
fred %>% 
  inner_join(banguat) %>% 
  inner_join(secmca) %>%
  write_csv(here("data", "main dataset.csv"))
```

...
Por otro lado, este estudio aborda la aplicación de cinco algoritmos de aprendizaje supervisado. El primero de ellos, la regresión de máquinas de vectores de soporte (SVR), se basa en un procedimiento en el que los datos son proyectados, dado un espacio de características con $p$ dimensiones, en un hiperplano (un subespacio de dimensiones $p-1$) en el que las observaciones pueden ser linealmente segregables. Posteriormente a la proyección de los datos en este nuevo espacio de características, el algoritmo encuentra la línea que tenga la mayor distancia vertical entre sus observaciones más cercanas; estos puntos más cercanos al hiperplano son llamados vectores de soporte [@parbat2020].

El problema de optimización consiste entonces en encontrar el margen máximo que separa al hiperplano, clasificando correctamente tantas observaciones como sea posible. Por lo que una regresión de vectores de soporte se formula definiendo primero una función de pérdida convexa insensible a ser minimizada y encontrando el tubo más plano que contiene la mayoría de las instancias de entrenamiento [@smola2004].

bosques aleatorios (RF), algoritmos de potenciación de gradiente extremo (XGB), perceptrones multicapa (MLP) y redes neuronales recurrentes de larga memoria de corto plazo (LSTM)

...
hurst %<>% 
  cbind(main_longer %>% distinct(Variable)) %>% 
  select(Variable, Hurst = X0.833747855107657) %>% 
...
granger_causality <- function(data, x, y, lag) {
  x <- 
    data %>% 
    select(!!enquo(x)) %>% 
    pull() %>% 
...
names_two <- c("cpi", "cred", "deprate")

list <- list()
granger <- data.frame()

for (i in names_two) {
  list <- granger_causality(main, x = ipc, y = all_of(i), lag = 1)
  granger <- rbind(granger, list)
}
...
for (i in 1:48) {
  list <- granger_causality(main, x = ipc, y = tpm, lag = i)
  granger <- rbind(granger, list)
}
...
$^1$ El nivel de significancia se encuentra entre paréntesis.
